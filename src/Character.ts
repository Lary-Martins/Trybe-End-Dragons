...
class Character implements Fighter {
  ...
  levelUp(): void {
    const maxLifePointsCharacter = this._maxLifePoints + getRandomInt(1, 10);

    if (maxLifePointsCharacter > this._race.maxLifePoints) {
      this._maxLifePoints = this._race.maxLifePoints;
    }

    this._maxLifePoints = maxLifePointsCharacter;
    this._strength += getRandomInt(1, 10);
